...
public class UserServiceUnitTest {
    ...
    void should_find_user_by_name(){
        //stub repo, linient()-> using for stub that is not called in method
       // lenient().when(userRepository.findByUserNameAndIsDeleted(anyString(), anyBoolean())).thenReturn(user);
        // anyString(), anyBoolean() we need to use together, can't be used anyString(), true or "name", anyBoolean
        when(userRepository.findByUserNameAndIsDeleted(anyString(), anyBoolean())).thenReturn(user);// both must match
    ...
    void should_throw_exception_when_user_not_found(){ //can be done in 2 different way
        //Template to verify any not found
...
//        assertEquals("User not found", throwable.getMessage());
    ...
    void should_save_user() {
        ...
        verify(keycloakService, atLeastOnce()).userCreate(any());// to make sure is called inside the service method
